...
>在字符串"string"前加r"string",表示原生字符串,字符串不进行转义解析

>正则匹配规则与python转义规则重复时,会先执行python转义在进行正则匹配造成误会

例如"\$"先进行转义,发现没有"\$"这个转义格式,再进行正则匹配
...
> 如果不指定次数,那么匹配元素的个数会尽可能多,直至上限

>非贪婪模式(懒惰模式): 让匹配重复的元字符尽可能少的向后匹配内容。
>如果不指定次数,那么匹配元素的个数会尽可能少,在符合规则范围内最少
...
![分组](mark_down_img/re.png)

![分组](mark_down_img/search_match_findall.png)
>match 与 search 的区别
 match 只能匹配开头位置,即匹配对象前面不能有其他字符
 serch 不是开头位置也能进行匹配
