...
> 在项目的内容足够大时,单纯的保存或者注释已经不能满足管理代码的要求
>>alicinya:
...
>* git是开源的，多在linix下使用，可以管理各种文件
>* git是分布式的项目管理工具(svn是集中式的)
>>	分布式:没有中央服务器,直接从另一台git主机下载代码;或是主动推送代码
>>	集中式:客户端不能相互传输数据,中央服务器是所有数据的集散地,分配管理权限.客户端数据先上传至中央服务器,在由其他客户端进行下载.
...
![分支合并](mark_down_img/merge.png)
>>冲突:分支修改了原有的文件,使master发生了变化,
如果合并的是新文件,会告诉你冲突已解决,但需要自行commit
还有一种冲突,两个分支都修改了原文件,会提示冲突自动合并失败,需手动修改冲突并提交修改结果
