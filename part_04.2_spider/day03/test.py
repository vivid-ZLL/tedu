...
class MaoyanSpider(object):
    ...
    def parse_html(self, url):
        ...
        one_list = []
        # [<Element dd at 0x7f787b669788>,
        #  <Element dd at 0x7f787b669c48>,
        # <Element dd at 0x7f787b669b48>]
        # one_list =  [('/html/592','name','actor','time'),(),(),.....]
        # for 循环遍历,获取每个电影信息
        for dd in dd_list:
            item['name'] = dd.xpath('.//p[@class="name"]/a/text()')[0].strip()
            # print(item['name'])  霸王别姬
            item['star'] = dd.xpath('.//p[@class="star"]/text()')[0].strip()
            # print(item['star'])  主演：张国荣,张丰毅,巩俐
            item['time'] = dd.xpath('.//p[@class="releasetime"]/text()')[0].strip()[5:15]
            # print(item['time'])  # 1993-01-01
            item['href'] = dd.xpath('.//a[@class="image-link"]/@href')[0].strip()
            # print(item['href'])

            list01 = [item['href'], item['name'], item['star'], item['time']]
            one_list.append(list01)
